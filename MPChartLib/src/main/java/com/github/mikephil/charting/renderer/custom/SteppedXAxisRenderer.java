...
public class SteppedXAxisRenderer extends SteppedAxisRenderer {

    protected XAxis mXAxis;

    public SteppedXAxisRenderer(ViewPortHandler viewPortHandler, XAxis xAxis, Transformer trans) {
...
//        computeAxisValues(min, max);
    ...
    protected void computeAxisValues(float min, float max) {
        Log.d("david", "computeAxisValues1: " + this);
//        mAxis.mEntries = new float[]{21f,22f,23f,0f,1f,2f,3f,4f,5f};
//        mAxis.mCenteredEntries = new float[]{21f,22f,23f,0f,1f,2f,3f,4f,5f};
...
//        computeSize();
    ...
    protected void drawLabels(Canvas c, float pos, MPPointF anchor) {
        ...
        float[] positions = new float[mXAxis.mEntryCount];

        // only fill x values
        final float totalWidth = mViewPortHandler.contentRight() - mViewPortHandler.contentLeft();
        final float widthUnit = totalWidth / mXAxis.mEntryCount;
        System.arraycopy(mXAxis.mEntries, 0, positions, 0, positions.length);

        mTrans.pointValuesToPixel(positions);

        for (int i = 0; i < positions.length; i ++) {

            float x = widthUnit * i + mViewPortHandler.contentLeft();
            Log.d("david", "drawLabels x: " + x);
